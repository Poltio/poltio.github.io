...
(function () {
    oct8ne.rewrite = {
        init: function () {
            ...
            window.emojiPicker = new EmojiPicker({
                ...
                assetsPath: '/api/img/plugins/emoji/',
            ...
            $('.emoji-picker-icon').attr("tabIndex", "0").attr("aria-label", "Add an emoji").attr("role", "button");

            $(".bottom-menu li").on("mouseenter", function () { //Menu bottom por default
                $(this).removeClass("faded").siblings().addClass("faded");
                $(".btn-start-video, .btn-start-audio").addClass("faded");
                var lay = $(this).attr("attr-lay");
                displayHoverAdvice(lay);
            });
            ...
            $(".bottom-menu ul").on("mouseleave", function () {
                $(this).children().removeClass("faded");
                $(".btn-start-video, .btn-start-audio").removeClass("faded");
                $("[class*='-advice']").fadeOut(100);
            });

            $("#oct8ne-conference-toolbar button").on("mouseenter", function () { //Parte de la conferencia menu bottom
                $(this).removeClass("faded").siblings().addClass("faded");
                $(".bottom-menu li").addClass("faded");
                var lay = $(this).attr("attr-lay");
                displayHoverAdvice(lay);
            });

            $("#oct8ne-conference-toolbar button").on("mouseleave", function () {
                $(this).siblings().removeClass("faded");
                $(".bottom-menu li").removeClass("faded");
                $("[class*='-advice']").fadeOut(100);
            });

            $("#minimize-by-menu , .form-header , .chat-header").on("click", function (e) { //Minimize by menú , formulario y en chat. Hace trigger a funcion de iframes-desktop. 
                e.stopPropagation();
                $(".minimize-chat").trigger("click");
            });

            $(".up-tutorial .icon-common-open-visor").on("click", function () {
                //triger de cerrar y triger de esconde capa
                $(".close-new-tutorial").trigger("click");
                $(".show-coviewer").trigger("click");
            });

            //$("").on("click", function () { /*Antes era minimize-coviewer en chat*/
            //    //Podria ahcer esto tb "$("#minimize-chat").trigger("click");" ??
            //    $(".chat-menu-options,.summary-form").hide();
            ...
            $(".powered-by-oct8ne-new").on("click", function () { //EVENTOS NUEVO VISOR
                $("#block-modal").attr("aria-modal", "true").show().animate({
                    top: "0px",
                    height: "495px",
                    opacity: "1"
                });
                ...
                setTimeout(function () {
                    $o.accessibility.trapFocus($o.accessibility.trapFocusPopups.poweredBy);
                }, 1000)
    ...
    $o.iframes.changeStatusIframe = function (newStatus, isOpening, clickOnPage) {
        ...
        switch (newStatus) {
            case $o.iframes.iframeStatus.CLOSE:
                ...
                $.post("Oct8ne/CloseSession", { tokenVisitor: tokenVisitor }, function (data) {
                    ...
                    $o.storage.local.set($o.core.localStorageTypes.LASTDISPLAY, $o.core.localStorageActions.REMOVE);
                    $o.storage.local.set($o.core.localStorageTypes.HIDENEWTUTORIAL, $o.core.localStorageActions.REMOVE);
                    $o.core.doPostMessageToApi("CLOSEIFRAME");
                });
                break;

            case $o.iframes.iframeStatus.SEARCH:
                $o.visitorSearch.enableSelectResult = true; //DEPRECATED ??
                $o.core.doPostMessageToApi("REMOVELAYBORDER"); //Debido a si entran por PhonePin "openCoviewerFromPin". Hay que asegurar que quitamos el borde.
                saveStatusIframeInCookie(isOpening, $o.iframes.iframeStatus.SEARCH);
                // $("#coviewer-minimized , #coviewer-minimized-no-agent").hide(0);
                $(".show-coviewer").addClass("js-reverse");
                $("#coviewer-wrapper , #bottom-menu").show(0);
                displayChatOrForm();
                $(".chat-tool-box").css("border-radius", "0 0 5px 0");
                $("#chat-wrapper").addClass("custom-chat-wrapper-full")
                $("#pending-products").hide();
                if (!$("#show-coviewer-chat").is(":visible")) {
                    $("#show-coviewer-chat").show();
                };
                goToActiveNode();
                if ($o.utils && $o.utils.disableShowCoviewerButton) {
                    $("#show-coviewer-chat").css("visibility", "visible");
                };
                $(".show-coviewer").attr("aria-expanded", "true");
                break;

            case $o.iframes.iframeStatus.VIEWER:
                $o.core.doPostMessageToApi("REMOVELAYBORDER");
                saveStatusIframeInCookie(isOpening, $o.iframes.iframeStatus.VIEWER);
                // $("#coviewer-minimized").hide(0);
                $(".show-coviewer").addClass("js-reverse");
                $("#coviewer-wrapper , #bottom-menu").show(0);
                displayChatOrForm();
                $(".chat-tool-box").css("border-radius", "0 0 5px 0");
            ...
            case $o.iframes.iframeStatus.CHAT:
                saveStatusIframeInCookie(isOpening, $o.iframes.iframeStatus.CHAT);
                $("#coviewer-wrapper, #coviewer-minimize").delay(500).hide(0);
                $(".show-coviewer").removeClass("js-reverse");
                $("#bottom-menu").show(0);
                displayChatOrForm();
                $(".chat-tool-box").css("border-radius", "0 0 5px 5px");
                $("#chat-wrapper").removeClass("custom-chat-wrapper-full");
                if ($o.utils && $o.utils.disableShowCoviewerButton) {
                    $("#show-coviewer-chat").css("visibility", "hidden");
                };
            ...
            case $o.iframes.iframeStatus.MINIMIZED:
                ...
                //else
                // {
                //    $("#coviewer-wrapper , #chat-wrapper, #email-us-form").fadeOut(400);
                //}
                //var layToDisplay = $o.storage.local.get($o.core.localStorageTypes.LASTDISPLAY);
                //if (layToDisplay === "chat") {
                //    $("#coviewer-minimized").delay(200).fadeIn(300);
                //} else {
                //    $("#coviewer-minimized-no-agent").delay(200).fadeIn(300);
                //}
            ...
            case $o.iframes.iframeStatus.PINCODE:
                $("#coviewer-wrapper , #chat-wrapper, #bottom-menu , #email-us-form").hide(0);
        ...
        //actualizamos estado chat
        if (typeof ($o.chat.canVisitorType) !== 'undefined') {
            $o.chat.toogleDenyVisitorTyping(!$o.chat.canVisitorType);
        }
    };

    function displayHoverAdvice(lay) {
        $("." + lay + "-advice").fadeIn(100);
    };


    function displayChatOrForm() {
        var layToDisplay = $o.storage.local.get($o.core.localStorageTypes.LASTDISPLAY);

        if (!layToDisplay) { //Es la primera vez!
            if ($o.core.agentsConnected) {
                $("#chat-wrapper").show(0);
                $o.storage.local.set($o.core.localStorageTypes.LASTDISPLAY, "chat");
            } else {
                $("#email-us-form").show(0);
    ...
    function saveStatusIframeInCookie(isOpening, statusIframe) {
        if (!isOpening) {
            $o.variablesVisitor.sendCookiesToApi($o.variablesVisitor.nameFunction.saveCookies, [{ cookie: $o.cookies.coviewerStatus, value: statusIframe }]);
        };
    };
  ...
  */
  ...
  var EmojiArea_WYSIWYG = function($textarea, id, options) {
    ...
    this.$editor.on("paste", function (e) {
      ...
      else if (window.clipboardData) {  //customizado para IE11 
          e.preventDefault();
          contentIE = window.clipboardData.getData('Text');
          if (window.getSelection && contentIE !== null) {
              window.getSelection().getRangeAt(0).insertNode(document.createTextNode(contentIE));
          }
      }      
