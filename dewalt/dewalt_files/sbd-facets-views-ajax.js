/**
 * @file
 * SBD Search Facets views AJAX handling.
 */
...
(function ($, Drupal, once) {
   ...
   */
  Drupal.behaviors.sbdFacetsViewsAjax = {
    attach: function (context, settings) {

      // Loop through all facets.
      $.each(settings.facets_views_ajax, function (facetId, facetSettings) {
        if (facetId === "categories_category" || facetId === "new_coming_soon") {
          return false;
        }
  ...
  var updateFacetsView = function (href, current_dom_id, view_path) {
    // Event handler for Views's own AJAX completion, so we can run Facets's
    // AJAX afterward.
    var afterViewsAjax = function (e, xmlhttprequest, ajaxObject) {
      if (ajaxObject.url.indexOf('/views/ajax') === 0) {
        updateFacetsBlocks(href);
        $(document).off('.updateFacetsView', afterViewsAjax);
      }
    };
    ...
    // Set up a temporary event handler to replace the facets after the Views AJAX sends incorrect versions.
    $(document).on('ajaxComplete.updateFacetsView', afterViewsAjax);
